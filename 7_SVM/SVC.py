...
class SupportVectorClassifier:
    ...
    def _gx(self, kij: np.ndarray, idx: int) -> float:
        gx = np.dot(kij[idx], self.y * self.alpha)[0] + self.b
        return gx

    def _Ei(self, i: int) -> float:
        return self._gx(self.K, i) - self.y[i]
    ...
    def _inner_loop(self, idx1: int) -> int:
        ...
        if ((r1 < -self.tol and alpha1 < self.C) or (r1 > self.tol and alpha1 > 0)):
            ...
            K_11 = self.K[idx1, idx1]
            K_22 = self.K[idx2, idx2]
            K_12 = self.K[idx1, idx2]
